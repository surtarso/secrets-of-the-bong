...
##TODO -> if player hits head, enter playerhasweed mode (siren chase)

#dialogs
onready var speech_array: Array = ["Loser...","...and a fag.","Punk ass.","Can you dance?",
...
var chase_speed = 60.0 # speed if player has weed bonus [leaf]
...
func _on_PlayerDetector_body_entered(_body):
	if not PlayerData.has_suit and not is_bribed and not no_search_mode: #if "godmode" is off, not bribed or resting
		...
		if PlayerData.coffee > 0 and PlayerData.roaches > 0:
			player.camera._shake(0.2,15,8) # duration, frequency, amplitude (Camera2D.gd)
...
# warning-ignore:integer_division
		 ...
		 # else if ------------PLAYER IS CLEAN---------- [has no roaches/weed]
			...
			if PlayerData.roaches == 0 and not PlayerData.has_weed:
				# gives player time to flee before being up to search again
				no_search_mode = true
				_speech_bubble()
...
func _random_hit_sound() -> AudioStreamPlayer2D:
	...
	return hit_sound_array[0] #returns the first one to the audio player
			
#-------------------------CHASE HITBOX 
func _on_PlayerChase_body_entered(_body):  #the hitbox behind the enemy
	if not PlayerData.has_suit: #if godmode not active
		# FLIP:
...
func _random_chase_sound() -> AudioStreamPlayer2D:
	...
	return chase_sound_array[0] #returns the first one to the audio player
...
func _physics_process(delta):
	...
	_player_has_suit() #disables hitboxes when player god-mode is active
...
func _movement():
	if player.smokeblow_raycast.get_collider() == self and not no_search_mode: #got hit by player smoke
...
func _player_has_suit():
		if PlayerData.has_suit: #player godmode active
